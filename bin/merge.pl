...
for my $code (keys %products) {
	#say $code;
	$to_dbh->do("INSERT INTO product (code, name, price) VALUES (?, ?, ?)", undef, $code, $products{$code}{from}{name}, $products{$code}{from}{price});
...
my $get_subscriptions_sth = $from_dbh->prepare("SELECT code FROM subscription, product WHERE subscription.uid=? and subscription.pid=product.id");
...
while (my $row = $get_user_sth->fetchrow_hashref) {
	...
	if (not $in_to) {
		my %row = %$row;
		my @values = @row{'email', 'password', 'register_time', 'verify_code', 'verify_time', 'name', 'admin', 'login_whitelist'};
		#print Dumper \@values;
		$to_dbh->do("INSERT INTO user (email, password, register_time, verify_code, verify_time, name, admin, login_whitelist) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", undef, @values);
		$check_user_sth->execute($row->{email});
	...
	while (my $subs = $get_subscriptions_sth->fetchrow_hashref) {
		...
		$to_dbh->do('INSERT INTO subscription (uid, pid) VALUES (?, ?)', undef, $in_to->{id}, $products{$subs->{code}}{new});
	}
}
